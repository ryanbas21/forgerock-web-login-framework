<script lang="ts">
  ...
  let message = callback.getMessage();
  let options = choice.getOptions();
  let sanitized = xss(message);
  let selected = choice.getDefaultOption(); // gets the index position

  $: {
    sanitized = xss(message);
    options = choice.getOptions();
    message = callback.getMessage();
    selected = choice.getDefaultOption();
  }
